...
.run([ "$rootScope", "$location", "loginService", ($rootScope, $location, loginService) ->
    routepermission = [ "/dashboard" ]
    
    $rootScope.$on('$routeChangeStart', () ->
      $location.path "pages/signin"  if routepermission.indexOf($location.path()) isnt "" and not loginService.islogged()
    )  
